package com.example.production_mes.dao;

import com.example.production_mes.entity.MountGuard;
import org.apache.ibatis.annotations.Param;

import java.util.List;

/**
 * 员工上岗表(MountGuard)表数据库访问层
 *
 * @author makejava
 * @since 2020-09-16 09:09:41
 */
public interface MountGuardDao {

    /**
     * 通过ID查询单条数据
     *
     * @param id 主键
     * @return 实例对象
     */
    MountGuard queryById(String id);

    /**
     * 查询指定行数据
     *
     * @param offset 查询起始位置
     * @param limit  查询条数
     * @return 对象列表
     */
    List<MountGuard> queryAllByLimit(@Param("offset") int offset, @Param("limit") int limit);


    /**
     * 通过实体作为筛选条件查询
     *
     * @param mountGuard 实例对象
     * @return 对象列表
     */
    List<MountGuard> queryAll(MountGuard mountGuard);

    /**
     * 新增数据
     *
     * @param mountGuard 实例对象
     * @return 影响行数
     */
    int insert(MountGuard mountGuard);

    /**
     * 修改数据
     *
     * @param mountGuard 实例对象
     * @return 影响行数
     */
    int update(MountGuard mountGuard);

    /**
     * 通过主键删除数据
     *
     * @param id 主键
     * @return 影响行数
     */
    int deleteById(String id);

    List<MountGuard> query(String type, String spec, String id);

    List<MountGuard> queryByName(int i, int i1, String employeename);
}